...
let ModuleLatestVideos = (function() {
  ...
  let getNextVideos = (nextPageToken) => {
    ...
    $(document).ready(function() {
      ...
      $.getJSON(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelIdIda}&order=date&pageToken=${nextPageToken}&type=video&key=${youTubeKey}`, function(response) {
        $.each(response.items, function(i, item) {
          let html = `
          <div class="center">
            <p>${item.snippet.title}</p>
