...
let ModuleLatestVideos = (function() {
  let getTheLatestVideos = () => {
    ...
    $(document).ready(() => {
      ...
      $.getJSON(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelIdIda}&order=date&type=video&key=${youTubeKey}`)
        .done((response) => {
          // progress bar
          $.LoadingOverlay("hide");
  ...
  let getNextVideos = (nextPageToken) => {
    ...
    $(document).ready(function() {
      ...
      $.getJSON(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelIdIda}&order=date&pageToken=${nextPageToken}&type=video&key=${youTubeKey}`)
        .done((response) => {
          // progress bar
          $.LoadingOverlay("hide");
          $.each(response.items, function(i, item) {
...
let ModulePlaylists = (function() {
  let getPlaylists = () => {
    ...
    $(document).ready(function() {
      ...
      $.getJSON(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=PL4nEz_ACMgWUUN5AAR7tWNWbiipW3N2yH&key=${youTubeKey}`)
        .done((response) => {
          // progress bar
          $.LoadingOverlay("hide");
          $.each(response.items, function(i, item) {
  ...
  let getYouTubeUserDescussion = () => {
    youTubeComments.map(() => {
      ...
      $(document).ready(function() {
        ...
        $.getJSON(`https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&channelId=${channelIdIda}&key=${youTubeKey}`)
          .done((response) => {
            // progress bar
            $.LoadingOverlay("hide");
            $.each(response.items, function(i, item) {
