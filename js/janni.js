...
const channelIdJanni = "UCdyTmTQTyGA2I7mMBODzqig";
...
let ModuleLatestVideos = (function() {
  let getTheLatestVideos = () => {
    ...
    $(document).ready(function() {
      ...
      $.getJSON(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelIdJanni}&order=date&type=video&key=${youTubeKey}`, function(response) {
...
let ModulePlaylists = (function() {
  let getPlaylists = () => {
    ...
    $(document).ready(function() {
      ...
      $.getJSON(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=PLS2NsUi0c62VfGbNe4OuqJy0Ms-W8N7A4&key=${youTubeKey}`, function(response) {
        $.each(response.items, function(i, item) {
          let html = `
          <div class="center">
            <p>${item.snippet.title}</p>
